...
import { Package, Users, ShoppingBag, TrendingUp, Edit, Trash2 } from "lucide-react";
import { toast } from "@/hooks/use-toast";

const stats = [
  { label: "Total Products", value: products.length, icon: Package, color: "text-primary" },
  { label: "Categories", value: "6", icon: ShoppingBag, color: "text-primary" },
  { label: "Avg Rating", value: "4.7", icon: TrendingUp, color: "text-primary" },
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="p-6 border-b border-border flex items-center justify-between">
            ...
            <button
              onClick={() => toast({ title: "Coming soon", description: "Product management with database integration is being set up." })}
                    ...
                    <tr key={product.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => toast({ title: "Coming soon", description: "Edit functionality will be available with database integration." })}
                            className="p-2 text-muted-foreground hover:text-foreground transition-colors"
                          >
                            <Edit className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => toast({ title: "Coming soon", description: "Delete functionality will be available with database integration." })}
